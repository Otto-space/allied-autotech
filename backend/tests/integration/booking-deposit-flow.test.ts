...
describe.skipIf(!runDatabaseTests)(
  "owner booking requests and legacy deposit protection",
  () => {
    afterAll(async () => prisma.$disconnect());

    it("serializes a requested slot, requires staff confirmation, and invalidates reminders on reschedule", async () => {
      ...
      const [staffSession, firstSession, secondSession] = await Promise.all([
      ...
      const create = (cookie: Awaited<ReturnType<typeof session>>, key: string) =>
        request(app).post("/api/v1/customers/bookings").set(mutation(cookie, key)).send({
          slotId,
          policyVersion: "owner-booking-request-v2",
      ...
      expect(successful.body.data.booking.depositAmountKobo).toBe(null);
      expect(successful.body.data.booking.status).toBe("REQUESTED");
      expect(successful.body.data.booking.depositPayment).toBeNull();
      ...
      const confirmation = await request(app)
        ...
        .send({
          status: "CONFIRMED",
          expectedVersion: 0,
          resourceReviewNote: "Synthetic resources checked and available",
        });
      ...
      expect(confirmed.body.data.reminders).toHaveLength(1);
      expect(confirmed.body.data.depositPaidAt).toBeNull();
      ...
      const replacement = await request(app)
        ...
        .send({
          ...
          serviceId: service.id,
          staffId: staff.staffProfile!.id,
          startsAt: new Date(startsAt.getTime() + 24 * 3_600_000).toISOString(),
        });
      ...
      expect(moved.body.data.reminders).toHaveLength(1);
      expect(moved.body.data.status).toBe("REQUESTED");
      ...
      const dueReminder = await prisma.bookingReminder.findFirstOrThrow({
        where: { bookingId },
      });
      expect(dueReminder.status).toBe("CANCELLED");
      await new BookingReminderWorker().runOnce(100);
      expect(
        await prisma.notification.count({
          where: { resourceId: bookingId, deduplicationKey: { contains: "reminder" } },
        }),
      ).toBe(0);
      ...
      const reconfirmed = await request(app)
        ...
        .send({
          ...
          resourceReviewNote: "Synthetic replacement resources checked",
        });
      ...
      const disruption = await request(app)
        ...
        .set(mutation(staffSession))
        .send({
          expectedVersion: reconfirmed.body.data.version,
