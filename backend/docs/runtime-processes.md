...
The API does not require Resend credentials because it only writes encrypted identity-email outbox
events. The identity worker does not require frontend, WebAuthn, Paystack, object-storage, session,
MFA, or asset-ticket secrets. Missing configuration fails process startup; enabled provider work is
