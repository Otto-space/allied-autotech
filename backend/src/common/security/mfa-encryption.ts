...
export function encryptPaymentCheckoutState(payload: unknown): EncryptedEnvelope {
  return encrypt(
    JSON.stringify(payload),
    "payment-checkout",
    env.OUTBOX_ENCRYPTION_KEY,
...
export function decryptPaymentCheckoutState<T>(envelope: EncryptedEnvelope): T {
  return JSON.parse(
    decrypt(
      envelope,
      "payment-checkout",
